...
「同じIDですが、問題文が異なります」と出た場合は、プレビュー上部の照合方法を確認する。列名が`ID`の場合は初期選択される。問題文でまとめたい場合は「IDを使わず問題文でプレビュー」で再確認する。この操作では選択肢など他の割り当てを保持し、データの適用は行わない。ID照合を続ける場合はCSVのIDを問題ごとに一意にする。表示される行番号はカード枚数ではなく、セル内の改行も含むCSV上の行番号。
